...
@section( 'pageTitle' )
Excluded Payroll Report Signatories
@endsection
...
<h1>
    Excluded Payroll <small>Signatories</small>
</h1>
@endsection
            ...
            <div class="box-body">
                <form id="pageForm" method="post" role="form">
                    <fieldset class="form-group">
                        <div>
                            <label class="">CERTIFIED: That the abovementioned employees are rendering services as of the signing date</label>
                        </div>
                        <div class="row">
                            <div class="col-md-6">
                                <div>
                    ...
                    <fieldset class="form-group">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <span class="input-group-addon no-border-radius" id="sizing-addon2"><span class="fa fa-user"></span></span>
                                    <select class="form-control no-border-radius" id="employee5" name="employee5" required style="width: 100%;">
                                        <option selected value="">
                                            --- SELECT FROM THE LIST ---
                                        </option>
                                        ...
                                        <optgroup label="Employees">
                                            @foreach( $empDetails as $item )
                                                <option value="{{ $item->employeeNumber }}" {{ $item->employeeNumber == $signatories[4]->employeeNumber ? 'selected' : '' }}>
                                                    {{ $item->fullName }}
                                                </option>
                                            @endforeach
                                        </optgroup>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group">
                                    <span class="input-group-addon addon-custom" id="sizing-addon2"><span class="fa fa-briefcase"></span></span>
                                    <input class="form-control input-group" id="position5" name="position5" required type="text" value="{{ $signatories[4]->position ?? '' }}">
                                </div>
                            </div>
                        </div>
                    </fieldset>
    ...
    $( function() {
        ...
        $( '#save' ).on( 'click', function () {
            ...
            $.ajax({
                ...
                type     : 'post',
                url      : '{{ config('app.root') }}/casual/report/signatories'
            }).done( function( data, status, xhr ) {
                try {
                    let jsonData = JSON.parse( xhr.responseText );

                    if ( jsonData.ajaxSuccess == 'ok' ) {
                        window.location = '{{ config('app.root') }}/casual/report/signatories';
                    } else {
                        showDialog(
