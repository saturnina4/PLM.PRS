        ...
        if( ctype_xdigit( $recordId ) && mb_strlen( $recordId ) === 32 ) {
            ...
            if( $recordCount > 0 ) {
                ...
                foreach ( $reportData as $data ) {
                    ...
                    $gsis = [
                        'gsis_lr' => 'LR', 'gsis_consolidated' => 'CONS',
                        'gsis_policy' => 'PL', 'gsis_emergency' => 'EMER',
                        'gsis_education' => 'EDUC'
                    ];
                    $pagIbig = [
                        'pi_premium' => '', 'pi_mpl' => 'MPL', 'pi_ecl' => 'ECL'
                    ];
                    $employeeRowCount = 0;
                    $gsisCount = 0;
                    $pagIbigCount = 0;
                    ...
                    $employeeRowCount = $gsisCount > $pagIbigCount ? $gsisCount : $pagIbigCount;
                    $employeeRowCount = $employeeRowCount == 0 ? 1 : $employeeRowCount;
                    ...
                    $excelObject->setActiveSheetIndex( $pageNumber )
                        ...
                        ->setCellValue( 'S' . $currentRow, $data['ded_otherBills'] == 0 ? '' : $data['ded_otherBills'] )
                        ->setCellValue( 'T' . $currentRow, $data['empNetSalary'] )
                        ->setCellValue( 'U' . $currentRow, ( $count + 1 ) );
                ...
                for ( $page = 1; $page <= $pageCount; ++$page ) {
                    ...
                    if ( $page != $pageCount ) {
                        ...
                        $excelObject->setActiveSheetIndex( $page - 1 )
                            ...
                            ->setCellValue( "S{$rowForTotals}", "=IF(SUM('PAGE 1:PAGE {$page}'!S{$startingRow}:S{$endRow})=0,\"\",SUM('PAGE 1:PAGE {$page}'!S{$startingRow}:S{$endRow}))" )
                        ...
                        $excelObject->setActiveSheetIndex( $page - 1 )
                            ...
                            ->setCellValue( "S{$rowForTotals}", "=IF(SUM('PAGE 1:PAGE {$pageCount}'!S{$startingRow}:S{$endRow})=0,\"\",SUM('PAGE 1:PAGE {$pageCount}'!S{$startingRow}:S{$endRow}))" )
